...
Midnight navy + champagne gold, Apple-style liquid glass on every section over a real ultra-HD Milky Way that drifts in slow motion, spring-eased motion with momentum scrolling, and an interactive 3D hero: a Pixar-style Faraz
...
| `app.js` | Momentum scrolling (Lenis), hero parallax, scroll-linked tech ribbon, glass light sweep and sheen, card lift and tilt, compact nav with a gliding liquid pill, active nav link, copy buttons |
...
- Only the outer panels use `backdrop-filter`; the panes inside them are plain translucent layers, which keeps scrolling smooth.
